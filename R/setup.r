...
Q2 <- read.csv(file.path(data_path,'Experiment_2.csv'), check.names = F) # read in dataset
...
B1 <- read.csv(file.path(data_path,'Experiment_1_B1.csv'), check.names = F) # read in dataset

# get databases from B2 classes
B2 <- read.csv(file.path(data_path,'Experiment_1_B2.csv'), check.names = F) # read in dataset

# get databases from B2 classes second round
B3 <- read.csv(file.path(data_path,'Experiment_1_B3.csv'), check.names = F) # read in dataset
...
#-------------------------------------------------------------------------------
# For experiment 1 only combine scales of short and long UPPS
#-------------------------------------------------------------------------------
...
Q1 <- subset(Q1, sub != '83') # participant only used the extreme values on each scale and the time to complete the questionnaire is too short to be reading the questions
Q1 <- subset(Q1, sub != '497') # participant only used the extreme values on each scale and the time to complete the questionnaire is too short to be reading the questions
Q1 <- subset(Q1, sub != '499') # participant only used the extreme values on each scale and the time to complete the questionnaire is too short to be reading the questions



#-------------------------------------------------------------------------------
# For experiment 2 impute missing data for the Yale Food Addiction questionnaire
#-------------------------------------------------------------------------------

# Create an alternative database for coding
col <- paste("mYFAS2[", 1:13, "]", sep = "")
codeyfas <- Q2[col] # Creates another data.base containg only YFAS data to code them while keeping the real item scores available in the original data.base
...
#----------------- Compute the subscales for CES-D in experimenet 1 ------------
...
#----------------- Compute the subscales for IAT subscales in experimenet 2------------
...
#demongrafics for COHS validation db
...
# mark na the black cell of genre in demo2
...
# let's name the items in a way that lavaan will also recognize
db.COHS.cfa <- db.COHS
